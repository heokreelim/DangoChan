...
- 덱 공유 게시글 작성
  - 작성한 덱을 첨부파일의 형태로 서비스 이용자들과 공유
  - 첨부파일을 수정할 시, 기존에 업로드한 첨부파일은 삭제되며 새로운 파일로 대체 업로드
    - 첨부 가능 확장자 제한 기능
    - 업로드 가능한 파일 크기(사이즈) 제한 기능
      - (3월 19일 기준 100MB 초과 시 파일 업로드 제한)
  - 게스트 로그인 상태로도 게시글 작성 가능
...
# 저장된 덱 기반 "카드 뒤집기 게임"
## 주요 기능
- 내가 저장한 덱 중 하나를 무작위로 선정
- 해당 덱 내 카드 중, 무작위로 단어와 의미가 맞는 카드를 최대 8개까지 선택하여, {단어, 의미} 쌍으로 이루어 게임카드를 형성하여 게임을 구성 (2025년 3월 19일 기준)
  - 덱을 구성하는 카드의 총 개수가 8개 미만의 경우, 해당 카드 개수만큼의 게임 카드 수 구성
- 게임 시작과 함께 게임 경과 시간을 카운트함
- 단어와 의미가 맞을 경우, 올바로 카드를 뒤집은 것으로 판정
- 모든 카드를 맞추었을 시, Mission clear 안내메시지 출력과 함께 게임 종료
  - 시간 타이머 종료
...
# DangoChan (단고짱) 개인화 서비스

### 일본어 단어장과 학습 기록을 관리하는 웹 서비스입니다.
단어 카드 플래시 학습, 학습 기록 통계, 출석 관리 등 사용자의 학습 동기를 높이는 다양한 기능을 제공합니다.
...
- 카테고리 및 덱(Deck) 관리 기능
  ...
  - 덱 삭제   
- 사용자 맞춤 출석 캘린더
  - 오늘 날짜와 유저 정보를 가져와 이번 달의 출석 정보를 조회하여 달력에 출력
- 학습 진행률 시각화 (원형 차트)
  - 유저가 소유한 덱의 학습 진척도를 기준에 맞춰 진척도를 화면에 표시
- 트로피 및 업적 시스템 (모티베이션 기능)
  - 연속 출석, 누적 출석, 일일 학습, 누적 학습 등의 기준으로 유저 경험 향상 도모 
- 마이페이지에서 프로필 관리 및 학습 통계 확인
  - 개인 맞춤 서비스를 위한 프로필 관리 및 업적 관리 지원 
